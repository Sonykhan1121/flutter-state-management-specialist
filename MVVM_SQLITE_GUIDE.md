# SQLite and MVVM — Bloc

This branch teaches MVVM roles using a catalog Bloc and smaller Cubits.

| Feature | ViewModel role | View binding |
| --- | --- | --- |
| Catalog | `presentation/view_models/movie_catalog_bloc.dart` → `MovieCatalogBloc` | Events in, `MovieCatalogState` out; `BlocBuilder` |
| Favorites | `presentation/view_models/favorites_cubit.dart` → `FavoritesCubit` | Command methods; `BlocSelector` for favorite + busy state |
| Trailer | `presentation/view_models/trailer_view_model.dart` → `TrailerCubit` | A screen-scoped `BlocProvider` and `BlocBuilder` |

A Bloc or Cubit **is** the ViewModel in this branch; adding a second ViewModel wrapper would duplicate state. `app.dart` owns stable `AppDependencies`, exposes repository contracts with `MultiRepositoryProvider`, and creates state managers with `MultiBlocProvider`. BlocProvider closes its state managers. The catalog rejects stale request results, and favorite/trailer commands check lifecycle state before emitting asynchronous results.
