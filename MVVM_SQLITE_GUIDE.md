# SQLite and MVVM — Riverpod

This branch teaches the same MVVM boundaries with Riverpod 3.

| Feature | ViewModel | View binding |
| --- | --- | --- |
| Catalog | `presentation/view_models/catalog_view_model.dart` → `MovieCatalogNotifier` | `ref.watch(movieCatalogProvider)` |
| Favorites | `presentation/view_models/favorites_view_model.dart` → `FavoriteMoviesNotifier` | Selected state + derived providers |
| Trailer | `presentation/view_models/trailer_view_model.dart` → `trailerViewModelProvider` | `FutureProvider.autoDispose.family`, `AsyncValue.when`, invalidate to retry |

`di/repository_providers.dart` owns the application dependencies through `ref.onDispose` and exposes repository interfaces. Tests override those providers. Catalog/favorites Notifiers act as ViewModels; immutable state snapshots drive rendering. The trailer is a read-only async ViewModel, so a FutureProvider is sufficient. `movie_providers.dart` is a small export file for convenient presentation imports. `WidgetRef` uses `context.mounted` after awaits; Notifier `ref.mounted` protects async state updates.
